...
Copy Creator 是一款轻量级的跨平台桌面效率工具（Windows / Linux），以悬浮窗形式呈现，关闭后自动驻留系统托盘。它集成了剪切板历史管理、内容库和快捷输入三大核心功能，帮助用户在日常工作中高效地保存、查找和复用内容。
...
> 命名说明：应用内功能名为「快捷输入」（英文 Quick Input）；代码与 IPC
> 命令沿用历史标识 `quick_input` / `phrases`（如 `get_all_phrases`、
> `phrases` 表），与界面名语义等价，新增文档统一使用「快捷输入」。
