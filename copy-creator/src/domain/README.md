...
全项目领域规则的**唯一事实源**。三个窗口（主窗口/径向菜单/新建窗口）的组件只做布局与交互，一切"这条记录是什么、能做什么、怎么显示"的判定都在本目录。守卫规则的编号清单见 `src/architectureGuard.test.ts`。
...
| 本地路径→**可显示/可播放 URL** | `mediaUrl.ts`（媒体服务 token 全项目仅此处可用；asset 协议已停用，图片/音视频统一走回环媒体服务） |
